...
exports.compileStylus = function (stylus, code, options) {
    ...
    var content = filecache.check(filename);
    if (content) {
    ...
    compiler.render(function (err, css) {
            if (err) {
                ...
                if (compiler.evaluator && compiler.evaluator.requireHistory) {
                    filecache.set(filename, Object.keys(compiler.evaluator.requireHistory), css);
                }
    ...
    if (less.version[0] >= 2) { // 2.0.0 and above
        less.render(code, options)
            .then(function (output) {
                filecache.set(filename, output.imports, output.css);
        ...
        parser.parse(code, function (error, tree) {
            ...
            try {
                ...
                var deps = {};
                walkTree(tree, deps);
                filecache.set(filename, Object.keys(deps), css);
                deferred.resolve(css);
            }
            catch (ex) {
                deferred.reject(ex);
